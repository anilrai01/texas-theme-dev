 ...
 *@package Buziness
 */
    $gallery_title       = buziness_get_option( 'gallery_title' );
    $gy_content_type     = buziness_get_option( 'gy_content_type' );
    $number_of_gy_column = buziness_get_option( 'number_of_gy_column' );
    $number_of_gy_items  = buziness_get_option( 'number_of_gy_items' );
    for( $i=1; $i<=$number_of_gy_items; $i++ ) :
        $gallery_page_posts[] = absint(buziness_get_option( 'gallery_page_'.$i ) );
    endfor;
    ?>
                            ...
                            <div class="featured-image">
                                <a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
                                    ...
                                    <div class="hover-effect">
                                        <a href="<?php the_permalink(); ?>" class="more-link" target="_blank"><i class="fa fa-link"></i></a>
                                    </div>
                                </a>
                            </div><!-- .featured-image -->
                        <?php endif; ?>
